...
public class StarEnemy : Enemy
{
    ...
    public override void MoveEnemy()
    {
        if(isMoving)
        {
            ...
            sinPos.z = 0f;
            transform.position = Vector2.MoveTowards(transform.position + (sinPos), moveTarget, Random.Range(speedMultiplier*0.1f, speedMultiplier*1.5f) * Time.deltaTime);
        }
        else
        {
    ...
    private void Shoot(Vector2 direction)
    {
        ...
        for (int i = 0; i < bulletAmountInt; i++)
        {
            ...
            SoundManager.instance.PlaySoundOnce("starShoot", Vector3.zero, this.gameObject, true);
